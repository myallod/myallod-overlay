...
# $Header: /var/cvsroot/gentoo-x86/net-analyzer/zabbix/zabbix-2.4.5.ebuild,v 1.2 2015/05/16 10:37:39 pacho Exp $
...
KEYWORDS="~amd64 ~x86"
IUSE="agent java curl debug debugjabber frontend ipv6 xmpp ldap libxml2 mysql openipmi oracle postgres proxy server ssh snmp sqlite odbc static"
...
src_prepare() {
	eautoreconf
	if use debugjabber; then
		epatch "${FILESDIR}"/2.4/patches/zbx.jabber.debug.patch
	fi
}
...
src_install() {
	...
	if use frontend; then
		...
		webapp_postinst_txt en "${FILESDIR}/"2.4/postinstall-en.txt
		webapp_configfile \
