...
function php_get_uri ()
{
	case "${1}" in
		...
		;;
		"php")
			echo "https://www.pccc.com/downloads/apache/current/${2}"
		;;
		"suhosin")
			echo "http://download.suhosin.org/${2}"
		;;
		"fpm")
			echo "http://php-fpm.org/downloads/${2}"
		;;
		"gentoo")
			echo "http://portage.macrolan.co.za/distfiles/${2}"
		;;
		"backports")
			echo "http://php52-backports.googlecode.com/files/${2}"
		;;
		*)
...
if [[ -n ${FPM_VERSION} ]]; then
	FPM_PATCH="php-${PV}-fpm-${FPM_VERSION}.diff"
	FPM_URI="$(php_get_uri fpm ${FPM_PATCH}.gz )"
fi

if [[ -n ${BACKPORTS_VERSION} ]]; then
	BACKPORTS_PATCH="php52-backports-security-${BACKPORTS_VERSION}.patch"
	BACKPORTS_URI="$(php_get_uri backports ${BACKPORTS_PATCH} )"
fi
...
IUSE="${IUSE} adabas +backports bcmath berkdb birdstep bzip2 calendar cdb cjk
	crypt +ctype curl curlwrappers db2 dbase dbmaker debug doc empress
	empress-bcs esoob exif fdftk frontbase +filter firebird fpm
